...
module.exports = (app) => {
  ...
  const {Business,Users,Services,Staffs,Products,Customers,Retailers,Suppliers,OrdersFromRetailors,Orders}=require("../models");
  ...
  router.get("/byId/:id", async (req, res) => {
    ...
    router.post("/bussinfor",async(req,res)=>{
      ...
      const {business_name,business_description,business_type,industry,location,contacts,address_line_1,latitude,longitude,city,state,country,status,UserId,email}=req.body;
    ...
    const buss_infor= await  Retailers.create({
  ...
  router.get("/currentorders", validateToken, async (req, res) => {

    const current_orders = await Retailers.findOne({ where: { UserId: req.user.id },include: [Users,OrdersFromRetailors]});
   
    //const current_orders = await OrdersFromRetailors.findAll({ where: { UserId: req.user.id },include: [Retailers,Suppliers]});
  ...
  router.get("/getdetails/:id",validateToken, async (req, res) => {
    ...
    const details_b = await Business.findOne({ where: { id: id },include: [Users,Staffs,Services,Customers,Products,Orders]});
  ...
  router.get("/allretailers", async (req, res) => {
  
    try {
   
    const best_rated = await Retailers.findAll({include: [Users,Customers,Products,Services]});
  ...
  router.get("/vendors_",validateToken, async (req, res) => {
  
   
   
    const vendors = await Business.findAll({include: [Users,Customers,Products,Services]});
