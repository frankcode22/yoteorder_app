...
const Op=sequelize.Op
  ...
  router.put("/updatecustomerbill/:sId",async(req,res)=>{
    ...
    const amount_received=req.body.amount_paid;

    const initial_paid=req.body.initialPayment;

   // const amount_paid=initial_paid+amount_received;

   const amount_paid=req.body.new_paid;

    const balance=bal-amount_received;
   


    const updated_bill = await RetailerSales.update({amount_paid,balance},{ where: {id: req.params.sId}});
  ...
  router.get("/allbills",validateToken,async (req, res) => {
    ...
    const bills = await RetailerSales.findAll({ where: {RetailersId: my_buss.id},include: [Customers]});
