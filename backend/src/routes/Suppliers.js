...
module.exports = (app) => {
  ...
  const {Business,Users,Services,Staffs,Products,Customers,Bookings,Suppliers,SupplyStores,Notifications,OrdersFromRetailors,Retailers,Orders}=require("../models");
  ...
  router.get("/mybusiness",validateToken,async (req, res) => {
    ...
    const my_buss = await Suppliers.findOne({ where: { UserId: req.user.id},include: [Users,Retailers,Customers,Products,Notifications,Bookings,OrdersFromRetailors]});
  ...
  router.put("/acceptretailerorder/:rId", async (req, res) => {
    ...
    const status_confirm = await OrdersFromRetailors.update({order_status,request_accepted_at},{ where: { id: id }});
  ...
  router.put("/declineretailerorder/:rId", async (req, res) => {

    let request_declined_at=new Date();
    const id = req.params.rId;
   
    const order_status='declined';
   
  
    const status_decline = await OrdersFromRetailors.update({order_status,request_declined_at},{ where: { id: id }});
