...
const { Bookings,Users,Orders,Customers,Products,OrderBids,Business,Staffs,OrdersFromRetailors,Suppliers,Services} = require("../models");
...
router.post("/retailercheckout", async (req, res) => {
...
lineItems.forEach(async (item)=> {
  ...
  supp_id=item.supplierId;

  OrdersFromRetailors.create({
    price: item.price,
    quantity_ordered: item.quantity,
    ProductId:item.productId,
...
let supplier = Suppliers.findOne({ where: { id: supp_id }})

console.log('THE SUPPLIER ID IS',supp_id);


// let formated_contact='+254'+phone_no.substring(1);
