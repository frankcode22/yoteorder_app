...
const express = require("express");
  ...
  const { Products,Users,Business,Suppliers,SupplyStores} = require("../models");
  ...
  router.post('/save_store', uploadMiddleware.upload, async (req, res) => {
  ...
  router.get("/itemsuppliers/:pname", async (req, res) => {
    ...
    //  let new_lng_less_one=parseFloat(lng_minus_one).toFixed(2)
     ...
     let item_list =await SupplyStores.findAll({where:{
      product_name:{[Op.like]:'%'+ name +'%'}},include:[Suppliers]}
