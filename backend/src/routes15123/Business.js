...
const {Business,Users,Services,Staffs,Products,Customers,Orders}=require("../models");
...
router.get("/byId/:id", async (req, res) => {
  ...
  router.post("/bussinfor",async(req,res)=>{
    ...
    const {business_name,business_description,business_type,industry,location,contacts,address_line_1,latitude,longitude,city,state,country,status,UserId}=req.body;
...
router.get("/getdetails/:id",validateToken, async (req, res) => {
  ...
  const details_b = await Business.findOne({ where: { id: id },include: [Users,Staffs,Services,Products,Orders]});
...
router.get("/bestRated", async (req, res) => {
  ...
  const best_rated = await Business.findAll({include: [Users,Products,Services]});
...
router.get("/vendors_",validateToken, async (req, res) => {
  ...
  const vendors = await Business.findAll({include: [Users,Products,Services]});
