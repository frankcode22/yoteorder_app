...
function ItemsOrdered(){
  ...
  useEffect(()=>{
   ...
   API.get('order/resupply_orders', { headers: { accessToken: localStorage.getItem("accessToken") } }).then((response) => {
    
   // console.log('AEFSDFENKALDHGB;JIOBK  WFAPDOVJWSFVK MWSFVJIAFDSV C',response.data)
...
setSales(response.data.OrdersFromRetailors)

console.log('DATA ITEM NOW',response.data.OrdersFromRetailors)
    ...
    {sales.map((value, key) => {
      return (
        ...
        <td>
            ...
            <td><span class="badge bg-warning">{value.SupplierId}</span></td>
            <td>{value.createdAt}</td>
            <td><span class="badge bg-info">{value.order_status}</span></td>
            ...
            <button type="button" onClick={() => {
                getSelectedCustomerBill(value.CustomerId,value.orderId,value.amount_paid);
                  }}  class="btn btn-success"><i class="fe fe-eye me-2"></i>View Order</button>
